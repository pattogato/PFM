...
final class FacebookManager: FacebookManagerProtocol {
    ...
    func getFacebookUserData(viewController: UIViewController) -> Promise<SocialUserData> {
        return Promise {
            ...
            { (loginResult) in
                ...
                switch loginResult {
                case .failed:
                    reject(SocialLoginError.LoginFailed)
                case .cancelled:
                    reject(SocialLoginError.LoginCancelled)
     ...
     */
    func completeUserData(user: SocialUserData,
                          accessToken: AccessToken) -> Promise<SocialUserData> {
        return Promise {
            fulfill, reject in
            ...
            request.add(
                GraphRequest(graphPath: "me",
                             ...
                             httpMethod: .GET, apiVersion: .defaultVersion),
                batchParameters: ["fields" : "id,name,email"])
            { (response, result) in
                ...
                case .success(let response):
                    guard let respDict = response.dictionaryValue,
                        ...
                        let email = respDict["email"] as? String else {
                            reject(SocialLoginError.InvalidData)
                            return
                    }
